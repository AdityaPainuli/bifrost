...
For block-capable streaming output rules, `stream_replay_duration_ms` sets the maximum time for replaying an allowed buffered stream to the client. Smaller streams may finish sooner. It defaults to `200`; set it to `0` to send all buffered events immediately. When multiple matched block-capable rules specify different values, Bifrost uses the largest value.
